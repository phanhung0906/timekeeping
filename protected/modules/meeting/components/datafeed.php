...
function addCalendar($st, $et, $sub, $ade){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `meeting` (`subject`, `starttime`, `endtime`, `isalldayevent`) values ('"
...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $color, $team, $responsible, $tz){
  $ret = array();
  $db = new DBConnection();
  $db->getConnection();
  $sql1 = "SELECT * FROM meeting WHERE (`StartTime` BETWEEN ".$st." AND ".$et." OR `EndTime` BETWEEN ".$st." AND ".$et.") AND `Location`= ".$loc;
  if ($st<=$et  ) {
    try{
      $sql = "insert into `meeting` (`subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`, `team`,`responsible`) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
      ...
      //echo($sql);
      if(mysql_query($sql)!=false && mysql_query($sql1)==false   ){
        $ret['IsSuccess'] = true;
        $ret['Msg'] = 'add success';
        $ret['Data'] = mysql_insert_id();
      }else{
...
function listCalendarByRange($sd, $ed){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "select * from `meeting` where `starttime` between '"
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $team, $responsible, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    ...
    case "adddetails":
        ...
        if(isset($_GET["id"]) && (int)$_GET['id']){
            $ret = updateDetailedCalendar($_GET["id"], $st, $et, 
                ...
                $_POST["Location"], $_POST["colorvalue"], $_POST['team'], $_POST['responsible'], $_POST["timezone"]);
        }else{
