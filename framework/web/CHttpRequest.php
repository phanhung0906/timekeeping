 ...
 *
 * CHttpRequest also manages the cookies sent from and sent to the admin.
 * By setting {@link enableCookieValidation} to true,
 * cookies sent from the admin will be validated to see if they are tampered.
 ...
 * @property array $preferredAcceptType The admin preferred accept type as an array map, e.g. array('type' => 'application', 'subType' => 'xhtml', 'baseType' => 'xml', 'params' => array('q' => 0.9)).
 * @property array $preferredAcceptTypes An array of all admin accepted types (as array maps like array('type' => 'application', 'subType' => 'xhtml', 'baseType' => 'xml', 'params' => array('q' => 0.9)) ) in order of preference.
 * @property string $preferredLanguage The admin preferred language.
 * @property array $preferredLanguages An array of all admin accepted languages in order of preference.
...
class CHttpRequest extends CApplicationComponent
{
	 ...
	 * Note, this feature requires that the admin client accepts cookie.
	...
	/**
	 * Returns the admin agent, null if not present.
	 * @return string admin agent, null if not present
	 */
	 ...
	 * @return string admin IP address
	 */
	...
	/**
	 * Returns the admin host name, null if it cannot be determined.
	 * @return string admin host name, null if cannot be determined
	 */
	...
	/**
	 * Returns information about the capabilities of admin browser.
	 * @param string $userAgent the admin agent to be analyzed. Defaults to null, meaning using the
	 * current User-Agent HTTP header information.
	 * @return array admin browser capabilities.
	...
	/**
	 * Returns admin browser accept types, null if not present.
	 * @return string admin browser accept types, null if not present
	 */
	 ...
	 * @return array the admin accepted MIME types.
	 */
	 ...
	 * @param array $a admin accepted MIME type as an array map
	 * @param array $b admin accepted MIME type as an array map
	...
	/**
	 * Returns an array of admin accepted MIME types in order of preference.
	 * Each array entry consists of a map with the type, subType, baseType and params, an array map of key-value parameters.
	 * See {@link parseAcceptHeader()} for a description of the array map.
	 * @return array the admin accepted MIME types, as array maps, in the order of preference.
	 */
	...
	/**
	 * Returns the admin preferred accept MIME type.
	 * The MIME type is returned as an array map (see {@link parseAcceptHeader()}).
	 * @return array the admin preferred accept MIME type or false if the admin does not have any.
	 */
	...
	/**
	 * Returns an array of admin accepted languages in order of preference.
	 * The returned language IDs will NOT be canonicalized using {@link CLocale::getCanonicalID}.
	 * @return array the admin accepted languages in the order of preference.
	...
	/**
	 * Returns the admin preferred language.
	 * The returned language ID will be canonicalized using {@link CLocale::getCanonicalID}.
	 * @return string the admin preferred language or false if the admin does not have any.
	 */
	 ...
	 * <pre>
	 * <?php
	 *    Yii::app()->request->xSendFile('/home/admin/Pictures/picture1.jpg',array(
	 *        'saveName'=>'image1.jpg',
	 *        'mimeType'=>'image/jpeg',
	 *        'terminate'=>false,
	 *    ));
	 ...
	 * <ul>
	 * <li>saveName: file name shown to the admin, if not set real file name will be used</li>
