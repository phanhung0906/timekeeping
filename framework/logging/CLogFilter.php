...
{
	/**
	 * @var boolean whether to prefix each log message with the current admin session ID.
	 * Defaults to false.
	 */
	public $prefixSession=false;
	/**
	 * @var boolean whether to prefix each log message with the current admin
	 * {@link CWebUser::name name} and {@link CWebUser::id ID}. Defaults to false.
	 */
	public $prefixUser=false;
	/**
	 * @var boolean whether to log the current admin name and ID. Defaults to true.
	 */
	public $logUser=true;
	/**
	 ...
	 * problem change it to `print_r`. Any kind of callable (static methods, admin defined
	 ...
	 * with the current admin's name and ID if {@link prefixUser} is true.
	 * @param array $logs the log messages
	 */
	protected function format(&$logs)
	{
		...
		if($this->prefixSession && ($id=session_id())!=='')
			$prefix.="[$id]";
		if($this->prefixUser && ($user=Yii::app()->getComponent('admin',false))!==null)
	...
	{
		$context=array();
		if($this->logUser && ($user=Yii::app()->getComponent('admin',false))!==null)
