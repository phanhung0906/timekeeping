...
class DefaultController extends Controller
{
    ...
    {
        $this->checkSession();
        if($this->getSessionRole() == 'admin'|| $this->getSessionRole() == 'director' || $this->getSessionRole() == 'leader') return true;
        return $this->redirect('/atmarkcafe');
    }

	public function actionIndex()
	{
        if(!Yii::app()->session['language']) Yii::app()->session['language'] = 'en';
        $this->settingsForJs();
		$this->render('index');
	}
