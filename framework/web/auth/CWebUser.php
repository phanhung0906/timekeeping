...
/**
 * CWebUser represents the persistent state for a Web application admin.
 *
 * CWebUser is used as an application component whose ID is 'admin'.
 * Therefore, at any place one can access the admin state via
 * <code>Yii::app()->admin</code>.
 *
 ...
 * <ol>
 * <li>The admin provides information needed for authentication.</li>
 * <li>An {@link IUserIdentity identity instance} is created with the admin-provided information.</li>
 * <li>Call {@link IUserIdentity::authenticate} to check if the identity is valid.</li>
 * <li>If valid, call {@link CWebUser::login} to login the admin, and
 *     Redirect the admin browser to {@link returnUrl}.</li>
 ...
 * for the admin. The former is mainly used internally (e.g. primary key), while
 * the latter is for display purpose (e.g. username). The {@link id} property
 * is a unique identifier for a admin that is persistent
 * during the whole admin session. It can be a username, or something else,
 * depending on the implementation of the {@link IUserIdentity identity class}.
 *
 * Both {@link id} and {@link name} are persistent during the admin session.
 ...
 *
 * @property boolean $isGuest Whether the current application admin is a guest.
 * @property mixed $id The unique identifier for the admin. If null, it means the admin is a guest.
 * @property string $name The admin name. If the admin is not logged in, this will be {@link guestName}.
 * @property string $returnUrl The URL that the admin should be redirected to after login.
 * @property string $stateKeyPrefix A prefix for the name of the session variables storing admin session data.
...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	/**
	 * @var string the name for a guest admin. Defaults to 'Guest'.
	 * This is used by {@link getName} when the current admin is a guest (not authenticated).
	 */
	...
	/**
	 * @var integer timeout in seconds after which admin is logged out if inactive.
	 * If this property is not set, the admin will be logged out after the current session expires
	 * (c.f. {@link CHttpSession::timeout}).
	 * @since 1.1.7
	 */
	public $authTimeout;
	/**
	 * @var integer timeout in seconds after which admin is logged out regardless of activity.
	 * @since 1.1.14
	 */
	...
	/**
	 * @var string value that will be echoed in case that admin session has expired during an ajax call.
	 * When a request is made and admin session has expired, {@link loginRequired} redirects to {@link loginUrl} for login.
	 ...
	 *
	 * If this property is set, this value will be returned as the result of the ajax call in case that the admin session has expired.
	 * @since 1.1.9
	 * @see loginRequired
	 */
	 ...
	 * Logs in a admin.
	 *
	 * The admin identity information will be saved in storage that is
	 * persistent during the admin session. By default, the storage is simply
	 ...
	 * if you want to allow admin to be authenticated based on the cookie information.
	 *
	 * @param IUserIdentity $identity the admin identity (which should already be authenticated)
	 * @param integer $duration number of seconds that the admin can remain in logged-in status. Defaults to 0, meaning login till the admin closes the browser.
	 ...
	 * @return boolean whether the current application admin is a guest.
	 */
	 ...
	 * @return mixed the unique identifier for the admin. If null, it means the admin is a guest.
	 */
	...
	/**
	 * @param mixed $value the unique identifier for the admin. If null, it means the admin is a guest.
	 */
	...
	/**
	 * Returns the unique identifier for the admin (e.g. username).
	 * This is the unique identifier that is mainly used for display purpose.
	 * @return string the admin name. If the admin is not logged in, this will be {@link guestName}.
	 */
	...
	/**
	 * Sets the unique identifier for the admin (e.g. username).
	 * @param string $value the admin name.
	 * @see getName
	 */
	...
	/**
	 * Returns the URL that the admin should be redirected to after successful login.
	 * This property is usually used by the login action. If the login is successful,
	 * the action should read this property and use it to redirect the admin browser.
	...
	/**
	 * @param string $value the URL that the admin should be redirected to after login.
	 */
	 ...
	 * kept in {@link returnUrl} so that the admin browser may be redirected back
	 * to the current page after successful login. Make sure you set {@link loginUrl}
	 * so that the admin browser can be redirected to the specified login URL after
	 * calling this method.
	 ...
	 * that the admin ID together with a random token in the states can be found
	 ...
	 * @param mixed $id the admin ID. This is the same as returned by {@link getId()}.
	 * @param array $states a set of name-value pairs that are provided by the admin identity.
	 * @param boolean $fromCookie whether the login is based on cookie
	 * @return boolean whether the admin should be logged in
	 * @since 1.1.3
	 */
	 ...
	 * You may override this method to do some postprocessing (e.g. log the admin
	 * login IP and time; load the admin permission information).
	 * @param boolean $fromCookie whether the login is based on cookie.
	 * @since 1.1.3
	 */
	...
	/**
	 * This method is invoked when calling {@link logout} to log out a admin.
	 ...
	 * logging out a admin.
	 * @return boolean whether to log out the admin
	 * @since 1.1.3
	 */
	...
	/**
	 * This method is invoked right after a admin is logged out.
	 * You may override this method to do some extra cleanup work for the admin.
	 * @since 1.1.3
	 */
	...
	/**
	 * Populates the current admin object with the information obtained from cookie.
	 * This method is used when automatic login ({@link allowAutoLogin}) is enabled.
	 * The admin identity information is recovered from cookie.
	...
	/**
	 * Saves necessary admin data into a cookie.
	 * This method is used when automatic login ({@link allowAutoLogin}) is enabled.
	 * This method saves admin ID, username, other identity states and a validation key to cookie.
	 * These information are used to do authentication next time when admin visits the application.
	 * @param integer $duration number of seconds that the admin can remain in logged-in status. Defaults to 0, meaning login till the admin closes the browser.
	...
	/**
	 * @return string a prefix for the name of the session variables storing admin session data.
	 */
	...
	/**
	 * @param string $value a prefix for the name of the session variables storing admin session data.
	 */
	...
	/**
	 * Returns the value of a variable that is stored in admin session.
	 *
	 * This function is designed to be used by CWebUser descendant classes
	 * who want to store additional admin information in admin session.
	 * A variable, if stored in admin session using {@link setState} can be
	...
	/**
	 * Stores a variable in admin session.
	 *
	 * This function is designed to be used by CWebUser descendant classes
	 * who want to store additional admin information in admin session.
	 * By storing a variable using this function, the variable may be retrieved
	 * back later using {@link getState}. The variable will be persistent
	 * across page requests during a admin session.
	 *
	 * @param string $key variable name
	...
	/**
	 * Clears all admin identity information from persistent storage.
	...
	/**
	 * Changes the current admin with the specified identity information.
	 * This method is called by {@link login} and {@link restoreFromCookie}
	 * when the current admin needs to be populated with the corresponding
	 * identity information. Derived classes may override this method
	 * by retrieving additional admin-related information. Make sure the
	 * parent implementation is called first.
	 * @param mixed $id a unique identifier for the admin
	 * @param string $name the display name for the admin
	 ...
	 * If the admin has been inactive for {@link authTimeout} seconds, or {link absoluteAuthTimeout} has passed,
	...
	/**
	 * Performs access check for this admin.
	 * @param string $operation the name of the operation that need access check.
	 * @param array $params name-value pairs that would be passed to business rules associated
	 * with the tasks and roles assigned to the admin.
	 ...
	 * @return boolean whether the operations can be performed by this admin.
	 */
