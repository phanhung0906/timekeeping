 ...
 * CActiveForm will render the error in the page back to admin.</li>
 * <li>AJAX-based validation: when the admin enters data into an input field,
 ...
 * when the admin enters data into an input field,
 ...
 * all these validations will lead to the same admin interface changes and error
 * message content.
 *
 ...
 * Note that in order to make the latter two validations work, the admin's browser
 ...
 * or separately. For example, in a admin registration form, one may use AJAX-based
 * validation to check if the admin has picked a unique username, and use client-side
...
class CActiveForm extends CWidget
{
	 ...
	 * <li>validateOnType: boolean, whether to trigger an AJAX validation each time when the admin
