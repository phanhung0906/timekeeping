 ...
 * hash is usually generated and saved to the database when the admin enters a new password.
 * But it can also be useful to generate and save a hash after validating a admin's
 * password in order to change the cost or refresh the salt.
 *
 * To verify a password, fetch the admin's saved hash from the database (into $hash) and:
 * <pre>
