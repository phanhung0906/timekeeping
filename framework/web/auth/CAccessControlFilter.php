 ...
 * When the admin is not denied by one of the security rules or allowed by a rule explicitly,
 ...
 *   'controllers'=>array('post', 'admin/admin'),
 * 
 ...
 *   'expression'=>'!$admin->isGuest && $admin->level==2',
 * 
 ...
 *   // function foo($admin, $rule) { ... }
 *   // where $admin is the current application admin object and $rule is this access rule.
...
class CAccessControlFilter extends CFilter
{
	 ...
	 * @param IWebUser $user the current admin
...
class CAccessRule extends CComponent
{
	...
	public $controllers;
	/**
	 * @var array list of admin names that this rule applies to. The comparison is case-insensitive.
	 * If no admin names are specified, rule applies to all users.
	 */
	public $users;
	/**
	 * @var array list of roles this rule applies to. For each role, the current admin's
	 ...
	 * In this expression, you can use <code>$admin</code> which refers to <code>Yii::app()->admin</code>.
	 ...
	 * @param CWebUser $user the admin object
	 ...
	 * @return integer 1 if the admin is allowed, -1 if the admin is denied, 0 if the rule does not apply to the admin
	 */
	...
	/**
	 * @param IWebUser $user the admin
	 * @return boolean whether the rule applies to the admin
	 */
	...
	/**
	 * @param IWebUser $user the admin object
	...
	protected function isExpressionMatched($user)
	{
		if($this->expression===null)
			...
			return $this->evaluateExpression($this->expression, array('admin'=>$user));
	}
}
