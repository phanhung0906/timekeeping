...
EOD;
	...
	{
		$fileList['days.php']['callback']=array($this,'generateIndex');
		$fileList['index-mail.php']['callback']=array($this,'generateIndex');
	...
	/**
	 * Inserts path to framework's yii.php into application's days.php
	 *
	...
	public function generateIndex($source,$params)
	{
		...
		$yii=$this->getRelativePath($yii,$this->_rootPath.DIRECTORY_SEPARATOR.'days.php');
		$yii=str_replace('\\','\\\\',$yii);
		return preg_replace('/\$yii\s*=(.*?);/',"\$yii=$yii;",$content);
	}

	/**
	 * Inserts path to framework's yiit.php into application's index-mail.php
	 *
	 * @param string $source source file path
	 * @param array $params
	 * @return string modified source file content
