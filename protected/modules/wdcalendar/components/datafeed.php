...
function addDetailedCalendar($st, $et , $sub, $ade, $dscr, $loc, $color , $responsible, $tz){
  $ret = array();
  if ($st<=$et) {
    try{
      $db = new DBConnection();
      $db->getConnection();
      $sql = "insert into `jqcalendar` (`subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`, `responsible`) values ('"
        .mysql_real_escape_string($sub)."', '"
        .php2MySqlTime(js2PhpTime($st))."', '"
        .php2MySqlTime(js2PhpTime($et))."', '"
        .mysql_real_escape_string($ade)."', '"
      ...
      //echo($sql);
  		if(mysql_query($sql)!=false && $st>date("d-m-Y h:i:s ") ){
        $ret['IsSuccess'] = true;
        $ret['Msg'] = 'add success';
        $ret['Data'] = mysql_insert_id();
      }else{
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $responsible, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    ...
    case "adddetails":
        ...
        if(isset($_GET["id"]) && (int)$_GET['id']){
            $ret = updateDetailedCalendar($_GET["id"], $st, $et, 
                ...
                $_POST["Location"], $_POST["colorvalue"], $_POST['responsible'], $_POST["timezone"]);
        }else{
